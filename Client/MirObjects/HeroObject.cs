...
namespace Client.MirObjects
{
    public class HeroObject : PlayerObject//英雄
    {
        ...
        private void RefreshMirSetStats()//刷新天龙套设置属性
        {
            ...
            if (MirSet.Contains(EquipmentSlot.RingL) && MirSet.Contains(EquipmentSlot.RingR))//左戒指和右戒指
            {
                MaxMAC = (ushort)Math.Min(ushort.MaxValue, MaxMAC + 1);//最大防御+1
                MaxAC = (ushort)Math.Min(ushort.MaxValue, MaxAC + 1);//最大魔御+1
            }
            if (MirSet.Contains(EquipmentSlot.BraceletL) && MirSet.Contains(EquipmentSlot.BraceletR))//左手镯和右手镯
            {
                MinAC = (ushort)Math.Min(ushort.MaxValue, MinAC + 1);//最小防御+1
                MinMAC = (ushort)Math.Min(ushort.MaxValue, MinMAC + 1);//最小魔御+1
            }
            if ((MirSet.Contains(EquipmentSlot.RingL) | MirSet.Contains(EquipmentSlot.RingR)) && (MirSet.Contains(EquipmentSlot.BraceletL) | MirSet.Contains(EquipmentSlot.BraceletR)) && MirSet.Contains(EquipmentSlot.Necklace))//一个戒指 一个手镯 一个项链
            {
            ...
            if (MirSet.Contains(EquipmentSlot.RingL) && MirSet.Contains(EquipmentSlot.RingR) && MirSet.Contains(EquipmentSlot.BraceletL) && MirSet.Contains(EquipmentSlot.BraceletR) && MirSet.Contains(EquipmentSlot.Necklace))//双戒指 双手镯 项链
            {
            ...
            if (MirSet.Contains(EquipmentSlot.Armour) && MirSet.Contains(EquipmentSlot.Helmet) && MirSet.Contains(EquipmentSlot.Weapon))//盔甲 头盔 武器
            {
                MaxDC = (ushort)Math.Min(ushort.MaxValue, MaxDC + 2);
                MaxMC = (ushort)Math.Min(ushort.MaxValue, MaxMC + 1);
                MaxSC = (ushort)Math.Min(ushort.MaxValue, MaxSC + 1);
            ...
            if (MirSet.Contains(EquipmentSlot.Armour) && MirSet.Contains(EquipmentSlot.Boots) && MirSet.Contains(EquipmentSlot.Belt) && MirSet.Contains(EquipmentSlot.Helmet) && MirSet.Contains(EquipmentSlot.Weapon))//盔甲 靴子 腰带 头盔 武器
            {
                MinDC = (ushort)Math.Min(ushort.MaxValue, MinDC + 1);
        ...
        private void RefreshRedOrchidSetStats()//刷新虹膜套设置属性
        {
            ...
            if (RedOrchidSet.Contains(EquipmentSlot.BraceletL) | RedOrchidSet.Contains(EquipmentSlot.BraceletR))//左手镯或右手镯
            {
                HpDrainRate = (byte)Math.Min(byte.MaxValue, HpDrainRate + 2);//基础
            }
            if (RedOrchidSet.Contains(EquipmentSlot.RingL) | RedOrchidSet.Contains(EquipmentSlot.RingR))//左戒指或右戒指
            {
                HpDrainRate = (byte)Math.Min(byte.MaxValue, HpDrainRate + 10);//基础
            }
            if (RedOrchidSet.Contains(EquipmentSlot.BraceletL) && RedOrchidSet.Contains(EquipmentSlot.BraceletR))//左手镯 右手镯
            {
                HpDrainRate = (byte)Math.Min(byte.MaxValue, HpDrainRate + 2);//在基础上+2
            }
            if (RedOrchidSet.Contains(EquipmentSlot.RingL) && RedOrchidSet.Contains(EquipmentSlot.RingR))//左戒指 右戒指
            {
                HpDrainRate = (byte)Math.Min(byte.MaxValue, HpDrainRate + 10);//在基础上+10
            }
            if (RedOrchidSet.Contains(EquipmentSlot.Necklace) && (RedOrchidSet.Contains(EquipmentSlot.RingL) | RedOrchidSet.Contains(EquipmentSlot.RingR)) && (RedOrchidSet.Contains(EquipmentSlot.BraceletL) | RedOrchidSet.Contains(EquipmentSlot.BraceletR)))//项链 一个戒指 一个手镯
            {
                Accuracy = (byte)Math.Min(byte.MaxValue, Accuracy + 2);
            }
        }
        ...
        private void RefreshRedFlowerSetStats()//刷新魔血套设置属性
        {
            ...
            if (RedFlowerSet.Contains(EquipmentSlot.BraceletL) | RedFlowerSet.Contains(EquipmentSlot.BraceletR))//左手镯或右手镯
            {
                MaxHP = (ushort)Math.Min(ushort.MaxValue, MaxHP + 25);//HP上限+25
                MaxMP = (ushort)Math.Min(ushort.MaxValue, MaxMP - 25);//MP上限-25
            }
            if (RedFlowerSet.Contains(EquipmentSlot.RingL) | RedFlowerSet.Contains(EquipmentSlot.RingR))//左戒指或右戒指
            {
                MaxHP = (ushort)Math.Min(ushort.MaxValue, MaxHP + 25);//HP上限+25
                MaxMP = (ushort)Math.Min(ushort.MaxValue, MaxMP - 25);//MP上限-25
            }
            if (RedFlowerSet.Contains(EquipmentSlot.BraceletL) && RedFlowerSet.Contains(EquipmentSlot.BraceletR))//左手镯 右手镯
            {
                MaxHP = (ushort)Math.Min(ushort.MaxValue, MaxHP + 25);//HP上限+25
                MaxMP = (ushort)Math.Min(ushort.MaxValue, MaxMP - 25);//MP上限-25
            }
            if (RedFlowerSet.Contains(EquipmentSlot.RingL) && RedFlowerSet.Contains(EquipmentSlot.RingR))//左戒指 右戒指
            {
                MaxHP = (ushort)Math.Min(ushort.MaxValue, MaxHP + 25);//HP上限+25
                MaxMP = (ushort)Math.Min(ushort.MaxValue, MaxMP - 25);//MP上限-25
            }
            if (RedFlowerSet.Contains(EquipmentSlot.Necklace) && (RedFlowerSet.Contains(EquipmentSlot.RingL) | RedFlowerSet.Contains(EquipmentSlot.RingR)) && (RedFlowerSet.Contains(EquipmentSlot.BraceletL) | RedFlowerSet.Contains(EquipmentSlot.BraceletR)))//项链 一个戒指 一个手镯
            {

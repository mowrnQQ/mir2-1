...
namespace Server.MirObjects
{
    public sealed class PlayerObject : MapObject
    {
        ...
        private void AddItem(UserItem item)
        {
            ...
            if (item.Info.Type == ItemType.药剂 || item.Info.Type == ItemType.卷轴 || (item.Info.Type == ItemType.手稿 && item.Info.Effect == 1))
            {
                for (int i = 0; i < 4; i++)
                {
        ...
        private void StartGameSuccess()
        {
            ...
            ReceiveChat("欢迎来到此服务器.", ChatType.Hint);

            if (Settings.TestServer)
            {
                ReceiveChat("服务器现在是测试模式.", ChatType.Hint);
                Chat("@GAMEMASTER");
            }
            ...
            if (Info.GuildIndex != -1)
            {
                ...
                {
                    Info.GuildIndex = -1;
                    ReceiveChat("你被公会请出.", ChatType.System);
                }
                else
                {
            ...
            if ((Info.Mentor != 0) && (Info.MentorDate.GetValueOrDefault().AddDays(Settings.MentorLength) < DateTime.Now))
                MentorBreak();
            else
                GetMentor();
            ...
            if (Info.DoubleSlash) Enqueue(new S.SpellToggle { Spell = Spell.风剑术, CanUse = true });
            ...
            {
                double totalMinutes = (Envir.Now - Info.LastDate).GetValueOrDefault().TotalMinutes;
            ...
            SMain.Enqueue(string.Format("{0} 已连接.", Info.Name));
                ...
                for (int x = CurrentLocation.X - Globals.DataRange; x <= CurrentLocation.X + Globals.DataRange; x++)
                {
                    ...
                    for (int i = 0; i < cell.Objects.Count; i++)
                    {
                        ...
                        else if (ob.Race == ObjectType.Spell)
                        {
                            ...
                            if ((obSpell.Spell != Spell.爆阱) || (IsFriendlyTarget(obSpell.Caster)))
                                Enqueue(ob.GetInfo());
                        }
            ...
            else if (message.StartsWith("@"))
            {
                ...
                switch (parts[0].ToUpper())
                {
                    ...
                    case "RESTORE":
                        ...
                        ReceiveChat(string.Format("玩家 {0} 已经被恢复", data.Name), ChatType.System);
                        SMain.Enqueue(string.Format("玩家 {0} 已经被 {1} 恢复", data.Name, Name));
                    ...
                    case "CHANGEGENDER":
                        ...
                        SMain.Enqueue(string.Format("玩家 {0} 被 {2} 修改为 {1} ", data.Name, data.Gender, Name));
                    ...
                    case "LEVEL":
                        ...
                        if (parts.Length >= 3)
                        {
                            ...
                            if (ushort.TryParse(parts[2], out level))
                            {
                                ...
                                SMain.Enqueue(string.Format("玩家 {0} 被 {3} 升级 {1} -> {2}", player.Name, old, player.Level, Name));
                                return;
                            }
                        }
                        else
                        {
                            ...
                            if (ushort.TryParse(parts[1], out level))
                            {
                                ...
                                ReceiveChat(string.Format("升级 {0} -> {1}.", old, Level), ChatType.System);
                                SMain.Enqueue(string.Format("玩家 {0} 被 {3} 升级 {1} -> {2}", Name, old, Level, Name));
                                return;
                            }
                        }
                    ...
                    case "MAKE":
                        ...
                        ReceiveChat(string.Format("{0} x{1} 被制造.", iInfo.Name, tempCount), ChatType.System);
                        SMain.Enqueue(string.Format("玩家 {0} 制程 {1} x{2}", Name, iInfo.Name, tempCount));
                        break;
        ...
        public void Attack(MirDirection dir, Spell spell)
        {
            ...
            {
                magic = MoonLightAttack ? GetMagic(Spell.月影术) : GetMagic(Spell.烈火身);
        ...
        public void Magic(Spell spell, MirDirection dir, uint targetID, Point location)
        {
            ...
            {
                UserMagic booster = GetMagic(Spell.深延术);
            ...
            Direction = dir;
            if (spell != Spell.野蛮冲撞 && spell != Spell.风弹步 && spell != Spell.拔刀术)
                Enqueue(new S.UserLocation { Direction = Direction, Location = CurrentLocation });
        ...
        {
            UserMagic magic = GetMagic(Spell.气功术);
        ...
        public void ObtainElement(bool cast)
        {
            ...
            UserMagic spell = GetMagic(Spell.气功术);
        ...
        public void EquipSlotItem(MirGridType grid, long id, int to, MirGridType gridTo)
        {
        ...
        public void RemoveItem(MirGridType grid, long id, int to)
        {
        ...
        public void RemoveSlotItem(MirGridType grid, long id, int to, MirGridType gridTo)
        {
        ...
        public void EquipItem(MirGridType grid, long id, int to)
        {
                ...
                case ItemType.药剂:
                    switch (item.Info.Shape)
                    {
                        ...
                        case 2: //MysteryWater
                            if (UnlockCurse)
                            {
                                ReceiveChat("你已经可以脱下诅咒的装备了.", ChatType.Hint);
                                Enqueue(p);
                                return;
                            }
                            ReceiveChat("你现在可以脱下诅咒的装备了.", ChatType.Hint);
                            UnlockCurse = true;
                            break;
                        case 3: //Buff
        ...
        public void SplitItem(MirGridType grid, long id, uint count)
        {
            ...
            if (grid == MirGridType.Inventory && (temp.Info.Type == ItemType.药剂 || temp.Info.Type == ItemType.卷轴 || temp.Info.Type == ItemType.护身符 || (temp.Info.Type == ItemType.手稿 && temp.Info.Effect == 1)))
            {
                if (temp.Info.Type == ItemType.药剂 || temp.Info.Type == ItemType.卷轴|| (temp.Info.Type == ItemType.手稿 && temp.Info.Effect == 1))
                {
                    for (int i = 0; i < 4; i++)
                    {
        ...
        public void MergeItem(MirGridType gridFrom, MirGridType gridTo, long fromID, long toID)
        {
            ...
            if(tempTo.Info.Type != ItemType.饵 && (gridFrom == MirGridType.Fishing || gridTo == MirGridType.Fishing))
            {
                Enqueue(p);
                return;
            }
        ...
        public void CombineItem(long fromID, long toID)
        {
            ...
            if (tempFrom.Info.Type != ItemType.宝玉)
            {
                Enqueue(p);
                return;
            }
            ...
            switch (tempFrom.Info.Shape)
            {
                ...
                case 4: //orbs
                    ...
                    {
                        if (succeeded) tempTo.AttackSpeed = (byte)Math.Max(sbyte.MinValue, (Math.Min(sbyte.MaxValue, tempTo.AttackSpeed + tempFrom.Info.AttackSpeed + tempFrom.AttackSpeed)));
                    }
                    ...
                    {
                        if (succeeded) tempTo.Luck = (byte)Math.Min(sbyte.MaxValue, tempFrom.Info.Luck + tempTo.Luck + tempFrom.Luck);
                    }
                    else
                    {
        ...
        public void DropItem(long id, uint count)
        {
        ...
        private bool CanUseItem(UserItem item)
        {
            ...
            if (RidingMount && item.Info.Type != ItemType.卷轴 && item.Info.Type != ItemType.药剂)
            {
                return false;
            }
        ...
        public void RequestChatItem(long id)
        {
       ...
       public void TalkMonster(uint objectID)
        ...
        public void BuyItem(long index, uint count)
        {
            if (Dead) return;
        ...
        public void SellItem(long uniqueID, uint count)
        {
        ...
        public void RepairItem(long uniqueID, bool special = false)
        {
        ...
        public void ConsignItem(long uniqueID, uint price)
        {
        ...
        public bool Match(AuctionInfo info)
        {
            ...
            return (UserMatch || !info.Expired && !info.Sold) && ((MatchType == ItemType.无 || info.Item.Info.Type == MatchType) &&
        ...
        public void MarketBuy(long auctionID)
        {
            if (Dead)
            {
        ...
        public void MarketGetBack(long auctionID)
        {
            if (Dead)
            {
                Enqueue(new S.MarketFail { Reason = 0 });
        ...
        public void Awakening(long UniqueID, AwakeType type)
        {
        ...
        public void DowngradeAwakening(long UniqueID)
        {
        ...
        public void DisassembleItem(long UniqueID)
        {
        ...
        public void ResetAddedItem(long UniqueID)
        {
        ...
        public void AwakeningNeedMaterials(long UniqueID, AwakeType type)
        {
            if (type == AwakeType.None) return;
            ...
            for (int i = 0; i < Info.Inventory.Length; i++)
            {
                ...
                if (materialItem != null)
                {
                    if (materialItem.Info.Grade == item.Info.Grade &&
                        materialItem.Info.Type == ItemType.觉醒)
                    {
            ...
            for (int i = 0; i < Info.Inventory.Length; i++)
            {
                if (Info.Inventory[i] != null)
                {
                    if (Info.Inventory[i].Info.Grade == item.Info.Grade &&
                        Info.Inventory[i].Info.Type == ItemType.觉醒)
                    {
        ...
        public bool CreateGuild(string GuildName)
        {
            ...
            for (int i = 0; i < Settings.Guild_CreationCostList.Count; i++)
            {
                ...
                else
                {
                    ...
                    {
                        if ((Required.Item.Type == ItemType.矿物) && (item.CurrentDura / 1000 > Required.Amount))
                        {
                            count = 0;
                            break;
                        }
                    ...
                    if (count != 0)
                    {
                        ...
                        {
                            if (Required.Item.Type == ItemType.矿物)
        ...
        public void SendMail(string name, string message, uint gold, long[] items, bool stamped)
        {
            CharacterInfo player = Envir.GetCharacterInfo(name);
            ...
            if (stamped)
            {
                for (int i = 0; i < Info.Inventory.Length; i++)
                {
                    ...
                    if (item == null || item.Info.Type != ItemType.无 || item.Info.Shape != 1 || item.Count < 1) continue;
        ...
        public void CollectMail(long mailID)
        {
            MailInfo mail = Info.Mail.SingleOrDefault(e => e.MailID == mailID);
        ...
        public void LockMail(long mailID, bool lockMail)
        {
            MailInfo mail = Info.Mail.SingleOrDefault(e => e.MailID == mailID);
        ...
        public uint GetMailCost(long[] items, uint gold, bool stamped)
        {
            uint cost = 0;
        ...
        private UserItem CreateDynamicWonderDrug(int boxtype, UserItem dropitem)
        {
            ...
            switch ((int)dropitem.Info.Effect)
            {
                case 0://exp low/med/high
                    dropitem.Luck = 5;
                    if (boxtype > 0) dropitem.Luck = 10;
                    if (boxtype > 1) dropitem.Luck = 20;
                    break;
                case 1://drop low/med/high
                    dropitem.Luck = 10;
                    if (boxtype > 0) dropitem.Luck = 20;
                    if (boxtype > 1) dropitem.Luck = 50;
                    break;
                case 2://hp low/med/high
                    dropitem.HP = (byte)50;
                ...
                case 5://amc low/med/high
                    ...
                    break;
                case 6://speed low/med/high
                    dropitem.AttackSpeed = 2;
                    if (boxtype > 0) dropitem.AttackSpeed = 3;
                    if (boxtype > 1) dropitem.AttackSpeed = 4;
                    break;
            }
        ...
        public void RefineItem(long uniqueID)
        {
        ...
        public void CheckRefine(long uniqueID)
        {
            //Enqueue(new S.RepairItem { UniqueID = uniqueID });
        ...
        public void ReplaceWeddingRing(long uniqueID)
        {
            if (Dead) return;
        ...
        public void MarriageRequest()
        {
            ...
            if (Info.MarriedDate.GetValueOrDefault().AddDays(Settings.MarriageCooldown) > DateTime.Now)
            {
            ...
            if (player != null)
            {
                ...
                if (player.Info.MarriedDate.GetValueOrDefault().AddDays(Settings.MarriageCooldown) > DateTime.Now)
                {
        ...
        public void GetRelationship(bool CheckOnline = true)
        {
            if (Info.Married == 0)
            {
                Enqueue(new S.LoverUpdate { Name = "", Date = Info.MarriedDate.GetValueOrDefault(), MapName = "", MarriedDays = 0 });
            }
            else
            {
                CharacterInfo Lover = Envir.GetCharacterInfo(Info.Married);
                ...
                if (player == null)
                    Enqueue(new S.LoverUpdate { Name = Lover.Name, Date = Info.MarriedDate.GetValueOrDefault(), MapName = "", MarriedDays = (short)(DateTime.Now - Info.MarriedDate).GetValueOrDefault().TotalDays });
                else
                {
                    Enqueue(new S.LoverUpdate { Name = Lover.Name, Date = Info.MarriedDate.GetValueOrDefault(), MapName = player.CurrentMap.Info.Title, MarriedDays = (short)(DateTime.Now - Info.MarriedDate).GetValueOrDefault().TotalDays });
                    if (CheckOnline)
                    {
                        player.GetRelationship(false);
        ...
        public void LogoutRelationship()
        {
            ...
            if (player != null)
            {
                player.Enqueue(new S.LoverUpdate { Name = Info.Name, Date = player.Info.MarriedDate.GetValueOrDefault(), MapName = "", MarriedDays = (short)(DateTime.Now - Info.MarriedDate).GetValueOrDefault().TotalDays });
